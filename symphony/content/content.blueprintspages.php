	...
	class contentBlueprintsPages extends AdministrationPage {
		...
		public function __actionEdit() {
			...
			if(@array_key_exists('save', $_POST['action'])) {
				 ...
				 */
				Administration::instance()->ExtensionManager->notifyMembers('PagePostValidate', '/blueprints/pages/', array('fields' => $fields, 'errors' => &$errors));
				...
				if(empty($this->_errors)) {
					...
					// Insert the new data:
						 ...
						 */
						Administration::instance()->ExtensionManager->notifyMembers('PagePreCreate', '/blueprints/pages/', array('fields' => &$fields));
						...
						if(!Symphony::Database()->insert($fields, 'tbl_pages')) {
							 ...
							 * @param array $fields
							 *  An assocative array of data that was just saved for this page
							 */
							Administration::instance()->ExtensionManager->notifyMembers('PagePostCreate', '/blueprints/pages/', array('page_id' => $page_id, 'fields' => &$fields));
					...
					// Update existing:
						 ...
						 *  The `$_POST['fields']` array passed by reference
						 */
						Administration::instance()->ExtensionManager->notifyMembers('PagePreEdit', '/blueprints/pages/', array('page_id' => $page_id, 'fields' => &$fields));
						...
						if(!Symphony::Database()->update($fields, 'tbl_pages', "`id` = '$page_id'")) {
							 ...
							 *  An assocative array of data that was just saved for this page
							 */
							Administration::instance()->ExtensionManager->notifyMembers('PagePostEdit', '/blueprints/pages/', array('page_id' => $page_id, 'fields' => $fields));
					 ...
					 * Use with caution as no futher processing is done on the `$types`
					 * array to prevent duplicate `$types` from occuring (ie. two index
					 ...
					 * @param array $types
					 *  An assocative array of the types for this page passed by reference.
					 */
					Administration::instance()->ExtensionManager->notifyMembers('PageTypePreCreate', '/blueprints/pages/', array('page_id' => $page_id, 'types' => &$types));
