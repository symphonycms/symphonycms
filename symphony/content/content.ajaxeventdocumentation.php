    ...
    class contentAjaxEventDocumentation extends TextPage
    {
        ...
        public function addEntrySuccessDoc(array &$doc_parts, $rootelement, $filters)
        {
            ...
            if ($this->hasMultipleFilter($filters)) {
                ...
                $entry = new XMLElement('entry', null,
                    array('index' => '0', 'result' => 'success', 'type' => 'create | edit'));
        ...
        {
            return new XMLElement('pre', '<code>' . str_replace('<', '&lt;',
                    str_replace('&', '&amp;', trim((is_object($code) ? $code->generate(true) : $code)))) . '</code>',
                array('class' => 'XML'));
        }
        ...
        public function addEntryFailureDoc(array &$doc_parts, $rootelement, $filters)
        {
            $doc_parts[] = new XMLElement('p',
                __('When an error occurs during saving, due to either missing or invalid fields, the following XML will be returned.'));

            if ($this->hasMultipleFilter($filters)) {
                $doc_parts[] = new XMLElement('p',
                    __('Notice that it is possible to get mixtures of success and failure messages when using the ‘Allow Multiple’ option.'));
                $code = new XMLElement($rootelement);
                ...
                $entry = new XMLElement('entry', null,
                    array('index' => '1', 'result' => 'success', 'type' => 'create | edit'));
                $entry->appendChild(new XMLElement('message', __('Entry [created | edited] successfully.')));
                $code->appendChild($entry);
            } else {
        ...
        public function addDefaultFiltersDoc(array &$doc_parts, $rootelement, $filters)
        {
            if (is_array($filters) && !empty($filters)) {
                $doc_parts[] = new XMLElement('p',
                    __('The following is an example of what is returned if any options return an error:'));

                $code = new XMLElement($rootelement, null, array('result' => 'error'));
                $code->appendChild(new XMLElement('message', __('Entry encountered errors when saving.')));
                $code->appendChild(new XMLElement('filter', null, array('name' => 'admin-only', 'status' => 'failed')));
                $code->appendChild(new XMLElement('filter', __('Recipient not found'),
                    array('name' => 'send-email', 'status' => 'failed')));
                $code->setValue('...');
        ...
        public function addFrontendMarkupDoc(array &$doc_parts, $rootelement, $section, $filters)
        {
            ...
            $doc_parts[] = new XMLElement('p',
                __('This is an example of the form markup you can use on your frontend:'));
            $container = new XMLElement('form', null,
                array('method' => 'post', 'action' => '{$current-url}/', 'enctype' => 'multipart/form-data'));
            $container->appendChild(Widget::Input('MAX_FILE_SIZE',
                (string)min(ini_size_to_bytes(ini_get('upload_max_filesize')),
            ...
            $doc_parts[] = self::processDocumentationCode(($multiple ? str_replace('fields[', 'fields[0][',
                $code) : $code));

            $doc_parts[] = new XMLElement('p',
                __('To edit an existing entry, include the entry ID value of the entry in the form. This is best as a hidden field like so:'));
            $doc_parts[] = self::processDocumentationCode(Widget::Input('id' . ($multiple ? '[0]' : null), '23',
                'hidden'));

            $doc_parts[] = new XMLElement('p',
                __('To redirect to a different location upon a successful save, include the redirect location in the form. This is best as a hidden field like so, where the value is the URL to redirect to:'));
        ...
        public function addSendMailFilterDoc(array &$doc_parts, $filters)
        {
            if ($this->hasSendEmailFilter($filters)) {
                ...
                $doc_parts[] = new XMLElement('p',
                    __('All of these fields can be set dynamically using the exact field name of another field in the form as shown below in the example form:'));
