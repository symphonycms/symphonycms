    ...
    class contentSystemPreferences extends AdministrationPage
    {
        ...
        public function view()
        {
            ...
            if (General::checkFile(CONFIG) === false) {
                $this->pageAlert(__('The Symphony configuration file, %s, or folder is not writable. You will not be able to save changes to preferences.',
                    array('<code>/manifest/config.php</code>')), Alert::ERROR);
                $bIsWritable = false;
            ...
            if (count($languages) > 1) {
                ...
                $group->appendChild(new XMLElement('p',
                    __('Authors can set up a differing language in their profiles.'), array('class' => 'help')));
            ...
            if (count($email_gateways) >= 1) {
                ...
                foreach ($email_gateways as $handle => $details) {
                    $options[] = array(
                        $handle,
                        (($handle === $default_gateway) || (($selected_is_installed === false) && $handle === 'sendmail')),
                        $details['name']
                    );
                }

                $select = Widget::Select('settings[Email][default_gateway]', $options,
                    array('class' => 'picker', 'data-interactive' => 'data-interactive'));
                $label->appendChild($select);
                $group->appendChild($label);
