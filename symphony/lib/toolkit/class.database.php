    ...
    class Database
    {
        ...
        public function __construct(array $config = array())
        {
            ...
            if (isset($config['pdo'])) {
                ...
                $this->connect(sprintf('%s:dbname=%s;host=%s;port=%d;charset=%s', $config['driver'], $config['db'],
        ...
        public function error(Exception $ex = null)
        {
            ...
            if (Symphony::ExtensionManager() instanceof ExtensionManager) {
                Symphony::ExtensionManager()->notifyMembers('QueryExecutionError',
                    class_exists('Administration') ? '/backend/' : '/frontend/', array(
            ...
            throw new DatabaseException(__('Database Error (%1$s): %2$s in query: %3$s',
                array($errornum, $msg, $this->_lastQuery)), array(
                'msg' => $msg,
                'num' => $errornum,
                'query' => $this->_lastQuery
            ), $ex);
        ...
        public function logQuery($query, $query_hash, $stop)
        {
             ...
             */
            if (Symphony::ExtensionManager() instanceof ExtensionManager) {
                Symphony::ExtensionManager()->notifyMembers('PostQueryExecution',
                    class_exists('Administration') ? '/backend/' : '/frontend/', array(
                        'query' => $query,
        ...
        public function fetch($query = null, array $params = array(), array $values = array())
        {
            if (!is_null($query)) {
                ...
                if (is_null($this->_lastResult) || $this->_lastResult === false) {
                    return array();
                }
            }
