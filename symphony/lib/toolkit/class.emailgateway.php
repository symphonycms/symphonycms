    ...
    abstract class EmailGateway
    {
        ...
        public function setTextEncoding($encoding = null)
        {
            if ($encoding === 'quoted-printable') {
                ...
                throw new EmailGatewayException(__('%1$s is not a supported encoding type. Please use %2$s or %3$s. You can also use %4$s for no encoding.',
                    array($encoding, '<code>quoted-printable</code>', '<code>base-64</code>', '<code>false</code>')));
            }
        }
        ...
        public function __set($name, $value)
        {
            if (method_exists(get_class($this), 'set' . $this->__toCamel($name, true))) {
                ...
                throw new EmailGatewayException(__('The %1$s gateway does not support the use of %2$s',
        ...
        {
            if (is_array($body)) {
                throw new EmailGatewayException(__('%s accepts strings only; arrays are not allowed.',
                    array('<code>appendHeaderField</code>')));
            }
            ...
            foreach ($this->_attachments as $key => $file) {
                ...
                if ($file_content !== false && !empty($file_content)) {
                    $output .= $this->boundaryDelimiterLine('multipart/mixed')
                        . $this->contentInfoString($file['mime-type'], $file['file'], $file['filename'],
                            $file['charset'])
                    ...
                    throw new EmailGatewayException(__('The content of the file `%s` could not be loaded.',
                        array($filename)));
                }
