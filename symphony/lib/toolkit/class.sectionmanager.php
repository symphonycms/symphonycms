<?php
    ...
    class SectionManager
    {
        ...
        {
            $details = Symphony::Database()->fetchRow(0, "SELECT `sortorder` FROM tbl_sections WHERE `id` = ?",
                array($section_id));
        ...
        {
            return Symphony::Database()->fetchVar('id', 0, "SELECT `id` FROM `tbl_sections` WHERE `handle` = ? LIMIT 1",
                array($handle));
        }
