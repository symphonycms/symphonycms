    ...
    class Entry
    {
        ...
        public function setDataFromPost($data, &$errors = null, $simulate = false, $ignore_missing_fields = false)
        {
            ...
            foreach ($schema as $info) {
                ...
                $result = $field->processRawFieldData((isset($data[$info['element_name']]) ? $data[$info['element_name']] : null),
                    $s, $message, $simulate, $this->get('id'));
        ...
        public function checkPostData($data, &$errors = null, $ignore_missing_fields = false)
        {
            ...
            foreach ($schema as $info) {
                ...
                if (Field::__OK__ !== $field->checkPostFieldData((isset($data[$info['element_name']]) ? $data[$info['element_name']] : null),
                        $message, $this->get('id'))
                ) {
                    $status = Entry::__ENTRY_FIELD_ERROR__;
                    $errors[$info['id']] = $message;
