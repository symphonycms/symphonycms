<?php
    ...
    class FieldTagList extends Field implements ExportableField, ImportableField
    {
        ...
        public function displaySettingsPanel(XMLElement &$wrapper, $errors = null)
        {
            ...
            foreach ($field_groups as $group) {
                ...
                foreach ($group['fields'] as $f) {
                    if ($f->get('id') !== $this->get('id') && $f->canPrePopulate()) {
                        $fields[] = array(
                            $f->get('id'),
                            (in_array($f->get('id'), $this->get('pre_populate_source'))),
                            $f->get('label')
                        );
                    }
                }
            ...
            $label->appendChild(Widget::Select('fields[' . $this->get('sortorder') . '][pre_populate_source][]',
                $options, array('multiple' => 'multiple')));
            $wrapper->appendChild($label);

            // Validation rule
            $this->buildValidationSelect($wrapper, $this->get('validator'),
                'fields[' . $this->get('sortorder') . '][validator]', 'input', $errors);
        ...
        public function commit()
        {
            ...
            $fields['pre_populate_source'] = (is_null($this->get('pre_populate_source')) ? 'none' : implode(',',
                $this->get('pre_populate_source')));
