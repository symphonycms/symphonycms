			...
			if(!is_array($data)){
				...
				if (!file_exists($file) || !is_readable($file)) {
					...
					if (isset($entry_id)) {
						...
						if (!empty($row)) {
							$result = $row;
							$file = WORKSPACE . $result['file'];
						}
					}

					// The current file is also missing:
					if (!file_exists($file) || !is_readable($file)) {
						$message = __('The file uploaded is no longer available. Please check that it exists, and is readable.');
		...
		public function appendFormattedElement(XMLElement &$wrapper, $data, $encode = false, $mode = null, $entry_id = null){
			...
			$item->setAttributeArray(array(
				'size' => (file_exists($file) && is_readable($file) ? General::formatFilesize(filesize($file)) : 'unknown'),
			 	'path' => General::sanitize(str_replace(WORKSPACE, NULL, dirname(WORKSPACE . $data['file']))),
				'type' => $data['mimetype'],
			));
