<?php
    ...
    class FieldDate extends Field implements ExportableField, ImportableField
    {
        ...
        public function displaySettingsPanel(XMLElement &$wrapper, $errors = null)
        {
            ...
            $input = Widget::Input('fields[' . $this->get('sortorder') . '][pre_populate]',
                $this->get('pre_populate') ? $this->get('pre_populate') : '', 'input');
        ...
        public function buildRangeFilterSQL($data, &$joins, &$where, $andOperation = false)
        {
            ...
            if ($andOperation) {
                foreach ($data as $date) {
                    ...
                    $start = ($date['start'] === self::$min_date) ? self::$min_date : DateTimeObj::getGMT('Y-m-d H:i:s',
                        $date['start']);
                    $end = ($date['end'] === self::$max_date) ? self::$max_date : DateTimeObj::getGMT('Y-m-d H:i:s',
                        $date['end']);
