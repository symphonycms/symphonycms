<?php
    ...
    class fieldTextarea extends Field implements ExportableField, ImportableField
    {
        ...
        public function displaySettingsPanel(XMLElement &$wrapper, $errors = null)
        {
            ...
            $div->appendChild($this->buildFormatterSelect($this->get('formatter'),
                'fields[' . $this->get('sortorder') . '][formatter]', __('Text Formatter')));
        ...
        public function displayPublishPanel(
            ...
            $textarea = Widget::Textarea('fields' . $fieldnamePrefix . '[' . $this->get('element_name') . ']' . $fieldnamePostfix,
                (int)$this->get('size'), 50, (strlen($value) !== 0 ? General::sanitize($value) : null));
        ...
        public function checkPostFieldData($data, &$message, $entry_id = null)
        {
            ...
            if ($this->__applyFormatting($data, true, $errors) === false) {
                $message = __('‘%s’ contains invalid XML.',
                        array($this->get('label'))) . ' ' . __('The following error was returned:') . ' <code>' . $errors[0]['message'] . '</code>';
