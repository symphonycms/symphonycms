<?php
    ...
    class ResourceManager
    {
        ...
        public static function __getExtensionFromHandle($type, $r_handle)
        {
            ...
            preg_match('/extensions\/(.*)\/' . $type . '/',
                call_user_func(array($manager, '__getClassPath'), $r_handle), $data);
        ...
        public static function getAttachedPages($type, $r_handle)
        {
            ...
            $pages = PageManager::fetch(false, array('id'), array(
                sprintf(
                    '`%s` = "%s" OR `%s` REGEXP "%s"',
                    $col,
                    $r_handle,
        ...
        public static function attach($type, $r_handle, $page_id)
        {
            ...
            $pages = PageManager::fetch(false, array($col), array(
                sprintf(
                    '`id` = %d',
                    $page_id
                )
