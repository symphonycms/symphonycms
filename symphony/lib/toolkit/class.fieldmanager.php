<?php
    ...
    class FieldManager implements FileResource
    {
        ...
        {
            return Symphony::Database()->fetchVar('type', 0, "SELECT `type` FROM `tbl_fields` WHERE `id` = ? LIMIT 1",
                array($id));
        }
        ...
        public static function fetch(
            ...
            if (!empty($field_ids) || is_null($id)) {
                ...
                %s",
                    ...
                    $where,
                    isset($field_ids) ? " AND t1.`id` IN(" . implode(',',
                            $field_ids) . ") " : " ORDER BY t1.`{$sortfield}` {$order}"
                );
        ...
        public static function create($type)
        {
            if (!isset(self::$_pool[$type])) {
                ...
                if (!file_exists($path)) {
                    throw new Exception(
                        __('Could not find Field %1$s at %2$s.',
                            array('<code>' . $type . '</code>', '<code>' . $path . '</code>'))
        ...
        public static function fetchHandleFromID($id)
        {
            return Symphony::Database()->fetchVar('element_name', 0,
                "SELECT `element_name` FROM `tbl_fields` WHERE `id` = ? LIMIT 1", array($id));
        }
        ...
        {
            $structure = General::listStructure(TOOLKIT . '/fields', '/field.[a-z0-9_-]+.php/i', false, 'asc',
                TOOLKIT . '/fields');
            $extensions = Symphony::ExtensionManager()->listInstalledHandles();
            $types = array();
        ...
        public static function isFieldUsed($field_type)
        {
            return Symphony::Database()->fetchVar('count', 0,
                "SELECT COUNT(*) AS `count` FROM `tbl_fields` WHERE `type` = ?", array($field_type)) > 0;
        }
        ...
        {
            $fields = Symphony::Database()->fetchCol('type',
                "SELECT DISTINCT `type` FROM `tbl_fields` WHERE `type` NOT IN ('author', 'checkbox', 'date', 'input', 'select', 'taglist', 'upload')");
