 ...
 */
/**
 * The HTTPGateway class provides a standard way to interact with HTTP requests.
 ...
 * $http->init('http://www.example.com/');
 * $http->setopt('POST', 1);
 * $http->setopt('POSTFIELDS', array('fred' => 1, 'happy' => 'yes'));
 * print $http->exec();
 * `
 */
class HTTPGateway
{
     ...
     * the `HTTPGateway::FORCE_SOCKET` constant as a parameter. The function
     ...
     *
     * @see toolkit.HTTPGateway#getInfoLast()
     * @param string $force_connection_method
     *  Only one valid parameter, `HTTPGateway::FORCE_SOCKET`
