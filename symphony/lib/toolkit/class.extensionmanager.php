<?php
    ...
    class ExtensionManager implements FileResource
    {
        ...
        public static function enable($name)
        {
            ...
            } elseif (($about = self::about($name)) && ($previousVersion = self::__requiresUpdate($name,
                    $about['version'])) !== false
            ) {
        ...
        public static function about($name, $rawXML = false)
        {
            ...
            if (file_exists(self::__getClassPath($name) . '/extension.meta.xml')) {
                ...
                if ($release = $xpath->query("//ext:release[@version='$latest_release_version']",
                    $extension)->item(0)
                ) {
                    $about += array(
                        'version' => $xpath->evaluate('string(@version)', $release),
                    ...
                    $current_symphony_version = preg_replace(array('/dev/i', '/-?beta\.?\d/i', '/-?rc\.?\d/i', '/.0/i'),
                        '', $current_symphony_version);
                    ...
                    // Min version
                    if (!empty($required_min_version) && version_compare($current_symphony_version,
                        ...
                        // Max version
                    } elseif (!empty($required_max_version) && version_compare($current_symphony_version,
        ...
        public static function fetchStatus($about)
        {
            ...
            if (isset($about['handle'], $about['version']) && self::__requiresUpdate($about['handle'],
                    $about['version'])
            ) {
        ...
        public static function cleanupDatabase()
        {
            ...
            if (is_array($rows) && !empty($rows)) {
                foreach ($rows as $r) {
                    ...
                    if (!@is_dir($path)) {
                        Symphony::Database()->delete("tbl_extensions_delegates", " `extension_id` = ?",
                            array($existing_id));
            ...
            if (is_dir(EXTENSIONS . "/{$extension_handle}/fields")) {
                foreach (glob(EXTENSIONS . "/{$extension_handle}/fields/field.*.php") as $file) {
                    ...
                    if (FieldManager::isFieldUsed($type)) {
                        throw new Exception(
                            __('The field ‘%s’, provided by the Extension ‘%s’, is currently in use.',
            ...
            if (is_dir(EXTENSIONS . "/{$extension_handle}/data-sources")) {
                foreach (glob(EXTENSIONS . "/{$extension_handle}/data-sources/data.*.php") as $file) {
                    ...
                    if (PageManager::isDataSourceUsed($handle)) {
                        throw new Exception(
                            __('The Data Source ‘%s’, provided by the Extension ‘%s’, is currently in use.',
                                array(basename($file), $about['name']))
            ...
            if (is_dir(EXTENSIONS . "/{$extension_handle}/events")) {
                foreach (glob(EXTENSIONS . "/{$extension_handle}/events/event.*.php") as $file) {
                    ...
                    if (PageManager::isEventUsed($handle)) {
                        throw new Exception(
                            __('The Event ‘%s’, provided by the Extension ‘%s’, is currently in use.',
                                array(basename($file), $about['name']))
                            . ' ' . __("Please remove it from your pages prior to uninstalling or disabling.")
                        );
                    }
            ...
            if (is_dir(EXTENSIONS . "/{$extension_handle}/text-formatters")) {
                foreach (glob(EXTENSIONS . "/{$extension_handle}/text-formatters/formatter.*.php") as $file) {
                    ...
                    if (FieldManager::isTextFormatterUsed($handle)) {
                        throw new Exception(
                            __('The Text Formatter ‘%s’, provided by the Extension ‘%s’, is currently in use.',
                                array(basename($file), $about['name']))
