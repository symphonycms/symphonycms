<?php
    ...
    class DynamicXMLDatasource extends Datasource
    {
        public function execute(array &$param_pool = null)
        {
            ...
            $stylesheet->setAttributeArray(array(
                'version' => '1.0',
                'xmlns:xsl' => 'http://www.w3.org/1999/XSL/Transform'
            ));

            $output = new XMLElement('xsl:output');
            $output->setAttributeArray(array(
                'method' => 'xml',
                'version' => '1.0',
                'encoding' => 'utf-8',
                'indent' => 'yes',
                ...
                if (Mutex::acquire($cache_id, $timeout, TMP)) {
                    ...
                    if ((int)$info['http_code'] !== 200 || !preg_match('/(xml|plain|text)/i', $info['content_type'])) {
                        ...
                        if ($info['curl_error'] === 28) {
                            ...
                            $result->appendChild(
                                new XMLElement('error',
                                    sprintf('Status code %d was returned. Content-type: %s', $info['http_code'],
                                        $info['content_type'])
                                )
                            );
                        }
                    ...
                    $result->appendChild(
                        new XMLElement('error',
                            __('The %s class failed to acquire a lock, check that %s exists and is writable.', array(
                                '<code>Mutex</code>',
                                '<code>' . TMP . '</code>'
                            )))
                    );
