...
final class EntryQueryFieldAdapterTest extends TestCase
{
    ...
    public function testCreateFilterEqualityOrNull()
    {
        ...
        $q->where($filter);
        $this->assertEquals(
            "SELECT FROM `entries` AS `e` WHERE (`f1`.`test` = :f1_test OR `f1`.`test` IS :_null_)",
            $q->generateSQL(),
    ...
    public function testSortAsc()
    {
        ...
        $o->sort($q, 'asc');
        $this->assertEquals(
            "SELECT `f1`.`value` FROM `entries` AS `e` LEFT JOIN `entries_data_1` AS `f1` ON `e`.`id` = `f1`.`entry_id` ORDER BY `f1`.`value` ASC , `e`.`id` ASC",
            $q->generateSQL(),
    ...
    public function testExactFilterSortAsc()
    {
        ...
        $o->sort($q, 'asc');
        $this->assertEquals(
            "SELECT `f1`.`value` FROM `entries` AS `e` LEFT JOIN `entries_data_1` AS `f1` ON `e`.`id` = `f1`.`entry_id` WHERE `f1`.`value` = :f1_value ORDER BY `f1`.`value` ASC , `e`.`id` ASC",
            $q->generateSQL(),
